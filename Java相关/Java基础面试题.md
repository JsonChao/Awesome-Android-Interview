...
#### 4、[静态代理和动态代理的区别，什么场景使用？](http://a.codekk.com/detail/Android/Caij/%E5%85%AC%E5%85%B1%E6%8A%80%E6%9C%AF%E7%82%B9%E4%B9%8B%20Java%20%E5%8A%A8%E6%80%81%E4%BB%A3%E7%90%86)
...
#### 9、arraylist和linkedlist的区别，以及应用场景

#### 10、TreeMap具体实现

#### 11、数组扩容

#### 12、List 和 Map 的实现方式以及存储方式。

#### 13、List,Set,Map的区别

#### 14、HashSet与HashMap怎么判断集合元素重复

#### 15、HashMap底层实现，hashCode如何对应bucket?

#### 16、hashMap的2倍扩容机制为什么是2倍

#### 17、在java8和java7中，hashMap的hash函数有什么不同

#### 18、ArrayList和LinkedList你知道吗？你知道它怎么动态扩容的吗？

#### 19、ArrayMap跟SparseArray在HashMap上面的改进
...
#### 5、为什么jdk8用metaspace数据结构用来替代perm？

#### 6、简单谈谈堆外内存以及你的理解和认识。

#### 7、怎么理解栈、堆？堆中存什么？栈中存什么？

#### 8、为什么要把堆和栈区分出来呢？栈中不是也可以存储数据吗？

#### 9、在Java中，什么是是栈的起始点，同是也是程序的起始点？
...
#### 13、Java中，栈的大小通过什么参数来设置？

#### 14、一个空Object对象的占多大空间？

#### 15、对象引用类型分为哪几类？

#### 16、java里带$的函数见过么，是什么意思

#### 17、服务器只提供数据接收接口，在多线程或多进程条件下，如何保证数据的有序到达；

#### 18、静态属性和静态方法是否可以被继承？是否可以被重写？以及原因？

#### 19、修改对象A的equals方法的签名，那么使用HashMap存放这个对象实例的时候，会调用哪个equals方法；
...
#### 26、int、char、long各占多少字节数

#### 27、int与integer的区别

#### 28、为什么Java里的匿名内部类只能访问final修饰的外部变量？
...
#### 40、Serializable 和Parcelable 的区别

#### 41、静态属性和静态方法是否可以被继承？是否可以被重写？以及原因？

#### 42、静态内部类的设计意图

#### 43、成员内部类、静态内部类、局部内部类和匿名内部类的理解，以及项目中的应用
...
#### 45、string 转换成 integer的方式及原理

#### 46、讲一下常见编码方式？

#### 47、utf-8编码中的中文占几个字节；int型几个字节？

#### 48、谈谈你对解析与分派的认识。

#### 49、如何将一个Java对象序列化到文件里？
...
#### 57、运行时栈帧结构（主要是局部变量表，理解栈堆）。

#### 58、Java重排序和顺序一致性。（as-if-serial和happens-before）
